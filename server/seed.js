// seed.js — fills the database with realistic Meridian data. Run with: node seed.js
const db = require('./db');

// Start clean so the script can be re-run safely
db.exec('DELETE FROM employees; DELETE FROM onboarding_tasks; DELETE FROM faq;');

// 15 employees across the 5 departments; HR is deliberately a single person,
// and is_buddy = 1 marks the go-to contact person in each department
const employees = [
...
const insertEmployee = db.prepare(
  'INSERT INTO employees (name, role, department, email, slack_handle, is_buddy) VALUES (?, ?, ?, ?, ?, ?)'
);
employees.forEach((e) => insertEmployee.run(...e));

// Onboarding tasks grouped in three phases: day 1, week 1, month 1
const tasks = [
  // Day 1
...
// FAQ for the First Day Guide page
const faqs = [
...
// Print a summary so we can confirm the seed worked
